...
export class AuthManager {
  ...
  async getValidStatePath(): Promise<string | null> {
    ...
    try {
      ...
      log.warning("⚠️  State file disappeared during validation");
      return null;
    }
  }
  ...
  async performSetup(sendProgress?: ProgressCallback, overrideHeadless?: boolean): Promise<boolean> {
    ...
    try {
      ...
      const errorDetail = error instanceof Error
        ? `${error.message}\n${error.stack}`
        : String(error);
      log.error(`❌ Setup failed: ${errorDetail}`);
   ...
   */
  async exportCookiesForAPI(): Promise<Array<{
    name: string;
    value: string;
    domain: string;
    path: string;
    ...
    try {
      if (!existsSync(this.stateFilePath)) {
        log.warning('⚠️  No state file found for cookie export');
        return [];
      }
      ...
      return state.cookies.map((c: any) => ({
        name: c.name,
        value: c.value,
        domain: c.domain,
        path: c.path,
   ...
   */
  async hasValidCookiesForAPI(): Promise<boolean> {
